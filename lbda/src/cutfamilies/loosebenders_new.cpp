...
LooseBenders::Cut LooseBenders::computeCut(arma::vec const &x)
{
    ...
    */
    ...
    for (size_t scenario = 0; scenario != d_problem.nScenarios(); ++scenario)
    {
        ...
        // extract cut coefficients of this scenario
        double    cur_delta = res;                             // current value of delta (constant in the cut)
        arma::vec cur_beta = - Tmat * dual;                    // current value of beta (x coefficient in the cut) //NOTE: minus sign taken care of
    ...
    std::vector<double> weights = risk_measure.compute_weights(d_problem.nScenarios());
    ...
    // TODO:
    // - find all places where objective function is implicitly computed using expected value
    //   - note: only used in subproblem, not in master
    ...
    // - also make changes to the deterministic equivalent (or anything else we want to compare the LBDA with)
